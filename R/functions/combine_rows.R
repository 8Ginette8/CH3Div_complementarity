...
combine_rows = function(df,ref.col=NULL,binary=FALSE){
	...
	out.cat = 
	lapply(u.sp,function(x){
		smry.cat = cat.info[cat.info[,ref.col]%in%x,]
		return(apply(smry.cat,2,function(x) length(table(x))))
	})
	...
	sp.out =
	lapply(u.sp,function(x){
		sp.tar = df[df[,ref.col]%in%x,]
		if (binary){
					
			# Keep continuous, multi.cat and uniq.cat columns separatly
			...
			if (any(w.cat%in%c("character","factor"))){
				...
				for (j in 1:ncol(cat.multi)){
					...
					if (any(tcat%in%1)) {tcat[is.na(tcat)] = 0}
			...
			for (j in 2:nrow(sp.tar)){sp.tar[1,][is.na(sp.tar[1,]) & i2] = sp.tar[j,][is.na(sp.tar[1,]) & i2]}
			val = apply(sp.tar[!i2],2,mean,na.rm=TRUE)
			out = data.frame(t(val),sp.tar[1,i2])
